...
This is a setup guide for gpu instance (rl-swarm)
## installation step for nvidia driver
```
...
sudo reboot
```
#### check if the driver has been installed successfully
```
nvidia-smi
```
...
### Fixing the error connection problem
If there are local changes, use
```
cd rl-swarm
git stash && git pull
```

Open, remove and add the required file
```
...
### editing the customized settings
If you wanna change the value of configs, open
```
...
```
nano rgym_exp/src/datasets.yaml
```
```
size: 1000
seed: 42
